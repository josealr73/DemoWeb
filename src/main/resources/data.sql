...
insert into empleado (id, nombre, apellidos)
	select 3, 'Lucía', 'Ricartis' from dual where not exists (select 1 from empleado where id = 3);
...
insert into usuario (username, nombre, password, rol_id)
	select 'user1', 'Paco Pepe', '$2a$10$5xOe75pbLcAjp0TbVWaluunrSshgYdH82YNwGd.b0Os4hAWbIEkry', 1 from dual where not exists (select 1 from usuario where username = 'user1');
